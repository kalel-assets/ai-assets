...
test('GitHub 저장소를 설치 명령이 없는 외부 자산으로 변환한다', () => {
  assert.deepEqual(toExternalAsset(repository, 'plugin'), {
    id: 'github-42',
    kind: 'plugin',
    source: 'external',
    name: 'agent-kit',
...
test('설명이 없고 topic이 없는 저장소에도 표시 가능한 기본값을 채운다', () => {
  assert.deepEqual(
    toExternalAsset({ ...repository, description: null, topics: [], license: null }, 'etc'),
    {
      id: 'github-42',
      kind: 'etc',
      source: 'external',
...
test('이름 정규화가 같은 서로 다른 GitHub 저장소에도 고유 ID를 부여한다', () => {
  const first = toExternalAsset({ ...repository, id: 41, full_name: 'octocat/agent_kit' }, 'skill')
  const second = toExternalAsset({ ...repository, id: 42, full_name: 'octocat/agent-kit' }, 'skill')
...
test('선택한 외부 자산을 저장하고 다시 복원한다', () => {
  ...
  const asset = toExternalAsset(repository, 'skill')
